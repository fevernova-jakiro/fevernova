...
@Slf4j
public class DTJobParser extends AbstractParser<Integer, TriggerResult> implements BarrierCoordinatorListener {


    protected ICheckPointSaver<MapCheckPoint> checkpoints = new CheckPointSaver<>();

    private TracingEngine<DTOrderBook, DTOrder> tracingEngine;

    private BinaryFileIdentity matchIdentity;


    public DTJobParser(GlobalContext globalContext, TaskContext taskContext, int index, int inputsNum, ChannelProxy channelProxy) {

        super(globalContext, taskContext, index, inputsNum, channelProxy);
        this.tracingEngine = new TracingEngine<>(new DTFactory(), this);
